...
| **H-04** | 75+ جدول مكشوف لـ `anon` عبر PostgREST | ⚠️ | — |
| **H-05** | `action_audit_log` سياسات متعارضة | ✅ | R05 |
| **M-01** | `user_roles` legacy لا يزال موجوداً | ✅ | R04 |
| **M-02** | `hr_inquiries` غير multi-tenant | ✅ | R01 |
| **M-03** | `invites` سياسة مكررة | ⏳ | — |
| **M-04** | `notifications.recipient_role` بدون CHECK | ⏳ | — |

**ملاحظة H-04:** R07 يُفعّل RLS على جميع الجداول مما يمنع المستخدمين المصادق عليهم بدور غير مخوَّل. لكن إيقاف الوصول الكامل لـ `anon` (غير المسجّلين) يتطلب `REVOKE SELECT ON ALL TABLES IN SCHEMA public FROM anon` — لم يُنفَّذ بعد.
...
| **sec** | **Security Hardening R00-R07:** ENUM rename + 12 RLS + SECURITY DEFINER + 23-table RLS | ✅ مُنجز |
| 11-12 | QMS Automation: quality_goals + nonconformance auto-reports | 🔄 جارٍ |
| **+1-2** | **Layer 5 Foundation:** workflow_instances + approval_gates + seed 5 workflows | ⏳ التالي |
| **+3-4** | **HR State Machine:** biometric webhook + cron + hr_accountability_tickets UI | ⏳ |
| **+5-6** | **Smart Wizards:** wizard_sessions + reason_codes_catalog + QF03-1 flow | ⏳ |
| **+7-8** | **Meeting Module:** meeting_sessions + Realtime + digital sign-off | ⏳ |
| **+9-10** | **Bulk Upload:** schema registry + validation Edge Function + approval | ⏳ |
...
*Sidra-ARR-v3.2 | رجب 1446هـ | يُلغي v1 وv2 — آخر تحديث: R07 يُعاد بناؤه وفق CLAUDE.md (NOT NULL، بلا backfill، DROP CASCADE)*

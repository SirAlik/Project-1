...
- **متبقٍّ معروف:** نمط `error.message` خام أوسع في خدمات **غير مُسمّاة** (`wizard`/`student-attendance`/`staff-evaluation`/`period-attendance`/`notification`/`bulk-upload`/`ai-service`) + `useStudentAffairs` + `_actions/{coordinator-classroom,academic-setup}` — تنظيف لاحق بنفس نمط `toSafeError`. **(أُغلق في Sprint 6.)**

---

## Sprint 6 — كنس `toSafeError` على مستوى المنصّة + تدقيق تقدّم المنهج (2026-06-27) — قواعد دائمة

> إغلاق `error.message` الخام المتبقي في الخدمات/الـhooks/الإجراءات التي لم تغطّها السبرنتات السابقة + تدقيق ميزة تقدّم المنهج للمعلّم. **app-code فقط — بلا migration**. lint صفر · build 63/63 · tsc نظيف · test 26/26. **بلا** لمس DB/RLS/migrations/Edge/auth/persona/مفاتيح الأدوار/التبعيات/`.env`. التقرير: `docs/audits/REMAINING_ITEMS_CLOSURE_REPORT.md` · تدقيق المنهج: `docs/audits/CURRICULUM_PROGRESS_FEATURE_AUDIT.md`.

- **كنس الرسائل الآمنة (المُسمّاة سابقاً كمتبقٍّ، أُغلقت الآن):** كل `error.message`/`*.message` خام يُعاد للمستخدم في `lib/services/{wizard,student-attendance,staff-evaluation,period-attendance,notification,bulk-upload,ai}-service.ts` (23 موضعاً) + `app/student-affairs/_hooks/useStudentAffairs.ts` (4 محمّلات عميل) + `app/_actions/{coordinator-classroom,academic-setup}.ts` (10 مواضع) — **37 موضعاً** صار يمرّ عبر `toSafeError(context, error, '<رسالة عربية آمنة>')` (أو رسالة inline آمنة + `console.error` حيث كان `console.error` قائماً مسبقاً، كـ `coordinator-classroom` ومسار advance في `wizard`). التفاصيل التقنية في `console.error` خادمياً فقط؛ شكل النتيجة (`ok`/`error`) بلا تغيير. **القاعدة الدائمة:** **ممنوع** إعادة `error.message`/`*.message`/`serverError`/JSON خام للمستخدم في أي خدمة/إجراء/hook — رسالة عربية آمنة عبر `toSafeError`، والخطأ يبقى ظاهراً (`ok:false`) لا يُبتلَع.
- **فروع أكواد الأخطاء المعروفة تبقى:** رسائل `error.code === '23505'/'23503'` العربية الواضحة في `academic-setup` (مضاف مسبقاً / مرتبط بسجلات) تبقى كما هي — هي رسائل مجال آمنة، والـ fall-through الخام فقط استُبدل.
- **تدقيق تقدّم المنهج للمعلّم → غير منفّذ (Missing):** لا جدول منهج/درس/وحدة (`information_schema` الحيّ: فقط `quest_progress` = gamification). الموجود placeholder ميت في تحليلات المدير (`useTeacherAnalytics`/`teachers/[id]`) مربوط ببيانات لا تُملأ أبداً (`individual=null`) وموجَّه للمدير لا للمعلّم؛ سطح المعلّم `/classroom/[classId]` بلا تقدّم منهج. **لا نسبة وهمية.** التصميم النظيف (3 جداول tenant-scoped + نسبة من `completed_lessons/total_lessons`) مُوثَّق كـ **Sprint 7 مقترح** — يحتاج migration ولم يُنفَّذ. **قاعدة:** حتى التنفيذ، لا تُعرض أي نسبة تقدّم منهج (صدق البيانات).
- **متبقٍّ معروف:** سطح تحليلات المدير القديم (`app/principal/analytics/teachers/*`) يحوي placeholders ميتة (سلاسل ثابتة «خطة الدرس»/«+%١٢» · صفحة عالقة على spinner) — تُنظَّف/تُربَط عند تنفيذ Sprint 7؛ لا تُعرَض للمستخدم حالياً (الصفحة لا تتجاوز التحميل). **(تقدّم المنهج نُفِّذ في Sprint 7.)**

---

## Sprint 7 — تقدّم المنهج للمعلّم (2026-06-27) — قواعد دائمة

> تنفيذ ميزة حقيقية لتقدّم المنهج داخل `/classroom/[classId]`. **app-code + 1 migration مُطبَّق على DB الحية** (`20260701_curriculum_progress.sql`). lint صفر · build 63/63 · tsc نظيف · test 26/26 · advisors **0 ERROR**. **بلا** تغيير auth/persona/مفاتيح الأدوار/التبعيات/`.env`. التفاصيل: `docs/audits/CURRICULUM_PROGRESS_FEATURE_AUDIT.md`.

- **الجداول (tenant-scoped · RLS · لا `USING(true)`):** `curriculum_units` (school_id+subject_id+grade_level+title+sort_order+is_active) · `curriculum_lessons` (school_id+unit_id+estimated_periods+is_active) · `class_curriculum_progress` (school_id+class_id+lesson_id+status['not_started'|'in_progress'|'completed']+completed_at+updated_by · **UNIQUE(class_id,lesson_id)**) + دالة `is_assigned_class_teacher(uuid)` (SECURITY DEFINER · search_path=public · EXECUTE لـ authenticated فقط).
- **النسبة حقيقية فقط:** `completedLessons / totalActiveLessons * 100` تُحسب من عدّ الدروس المكتملة فعلياً (في `CurriculumProgress.tsx`). إذا `total = 0` → 0% + حالة فارغة. **ممنوع** نسبة مُثبَّتة (20%/40%) أو قيمة نسبة مخزَّنة يدوياً أو placeholder مُختلَق.
- **عزل المستأجر إلزامي:** `school_id` من `getActivePersona()` خادمياً؛ كل RLS على `school_id = get_my_school_id()` (تقرأ مدرسة التوكن نفسه → عبر-المدارس مستحيل). المعلّم يقرأ/يُحدّث **فقط الفصول/المواد المُسنَدة إليه** (`is_assigned_class_teacher` + فحص `teacher_assignments` في الإجراء)؛ تأليف المنهج لـ school_admin/school_principal/academic_vp؛ system_owner قراءة فقط. تحقّق حيّ: helper=false لفصل غير مُسنَد (fail-closed).
- **الإجراءات في `app/classroom/[classId]/_actions.ts`:** `getClassCurriculumAction` (منهج+حالة لكل مادة) · `setLessonProgressAction` (يتحقّق الفصل/الدرس في المدرسة + صفّ الدرس=صفّ الفصل + تكليف المعلّم؛ upsert `onConflict:'class_id,lesson_id'`؛ `completed_at` فقط عند الإكمال). رسائل عربية آمنة + `console.error`.
- **قاعدة دائمة (المنهج):** أي قراءة/كتابة لجداول المنهج تمرّ عبر هذه الإجراءات (`school_id` خادمي)؛ النسبة تُشتقّ من عدّ حقيقي؛ **لا تُعرض نسبة بلا دروس مخزَّنة**. تأليف الوحدات/الدروس (UI إدارة المنهج) = عمل لاحق؛ حتى ذلك الحين الحالة الفارغة صادقة.
- **placeholders تحليلات المدير:** بقيت فارغة (لا تجميع على مستوى المعلّم)، واستُبدلت الملاحظة المضلّلة بحالة فارغة صادقة تشير لميزة الفصل. **لا نسبة منهج مُختلَقة متبقية.**
- **بيانات التجربة:** لا seed/demo معتمد؛ **لم تُزرع بيانات حيّة**. قائمة البيانات المطلوبة للتحقّق الحيّ بالمتصفّح موثّقة في `CURRICULUM_PROGRESS_FEATURE_AUDIT.md` (Phase 6). **(نموذج التأليف صُحِّح في Sprint 8 — الإدارة لا تؤلّف.)**

---

## Sprint 8 — توزيع المنهج يؤلّفه المعلّم (2026-06-27) — قواعد دائمة

> تصحيح خطأ تصميمي في Sprint 7: المنهج كان عالمياً يؤلّفه admin. **التصحيح الميداني:** الإدارة **تُسند** المادة/الفصل · المعلّم المُسنَد **يوزّع** المنهج · الإدارة **تتابع** فقط. **app-code + 1 migration تصحيحي مُطبَّق حياً** (`20260702_curriculum_distribution_correction.sql`). lint صفر · build 63/63 · tsc نظيف · test 26/26 · advisors **0 ERROR**. **بلا** تغيير auth/persona/مفاتيح الأدوار/التبعيات/`.env`. التفاصيل: `docs/audits/CURRICULUM_PROGRESS_FEATURE_AUDIT.md`.

- **تصحيح المخطّط (PRE-LAUNCH · 0 صفوف · drop+recreate نظيف):** `curriculum_units` أُعيد تحجيمها من `(subject_id, grade_level)` العالمي إلى **`(class_id, subject_id)`** + `created_by` · `curriculum_lessons` اكتسبت `planned_date`/`status`/`completed_at`/`notes` (الحالة على الدرس) · **حُذف** `class_curriculum_progress` (زائد) و`is_assigned_class_teacher` · القيد صار `UNIQUE(class_id, subject_id, title)` (لكل فصل توزيعه).
- **التأليف للمعلّم المُسنَد فقط (قاعدة دائمة):** كل إنشاء/تحرير/حذف لوحدة أو درس يمرّ عبر إجراءات `app/classroom/[classId]/_actions.ts` التي تتطلّب `role='teacher'` + تكليف (`teacher_assignments` لـ class+subject) + `school_id` خادمي. RLS: سياسات الكتابة لـ units/lessons للمعلّم المُسنَد فقط عبر `is_assigned_subject_teacher`/`is_unit_owner_teacher` (SECURITY DEFINER · search_path) — **لا سياسة كتابة للإدارة** (`admin_write_policies = null`، مُتحقَّق حياً). **ممنوع** جعل الإدارة المؤلِّف الافتراضي للمنهج.
- **الإدارة تتابع فقط:** `school_admin/school_principal/academic_vp` لهم SELECT فقط على الوحدات/الدروس (متابعة)؛ الإسناد عبر `teacher_assignments` (لم يُمَسّ). system_owner قراءة.
- **عزل المستأجر:** `school_id = get_my_school_id()` (مدرسة التوكن) → عبر-المدارس مستحيل · تحقّق حيّ: الدالتان تُعيدان false لغير المُسنَد (fail-closed).
- **النسبة حقيقية:** `completedLessons/totalLessons*100` من `status='completed'`؛ `total=0` → 0% + حالة فارغة «لم يضف المعلم توزيع المنهج لهذا الفصل بعد.» **لا نسبة وهمية.**
- **الواجهة:** `CurriculumProgress.tsx` صار سطح تأليف («توزيع المنهج»): إضافة وحدة/درس · تاريخ مخطّط · تحرير · حالة · حذف درس · حذف/تعطيل وحدة (تعطيل آمن إن كانت تحوي دروسًا) + شريط «تقدّم المنهج».
- **حذف آمن:** درس = حذف نهائي (ورقة)؛ وحدة بها دروس = تعطيل (`is_active=false`) لا حذف يفقد السجلّ.

...
**نطاق المراجعة:** db/migrations (37 ملف SQL) · lib/ · app/ · middleware.ts · جميع أنواع TypeScript
...
**لا. النظام في مرحلة بنية تحتية أساسية ولم يصل بعد إلى مستوى المنصة الذكية.**  
الكود الحالي يمتلك هيكلاً أمنياً جيداً (RLS + PBAC + تعدد الأدوار)، لكنه يفتقر إلى طبقة المنطق التشغيلي والربط الآلي التي تجعل الإدخال اليومي يتحول إلى مؤشرات وجودة وتحليلات.

### نسبة الجاهزية: **28 / 100**

| المحور | الجاهزية |
|--------|----------|
| البنية الأكاديمية الأساسية | 58% |
| الهوية / الأدوار / العلاقات | 52% |
| طبقة الأحداث الموحدة (Events Layer) | 22% |
| سيناريو الحضور اليومي وحضور الحصص | 15% |
| سيناريو الإحالات الآلية | 5% |
| سيناريو زيارات المكتبة (LRC) | 28% |
| سيناريو العيادة الصحية | 32% |
| سيناريو مغادرة الفصل | 8% |
| طبقة التحليلات والمؤشرات | 18% |
| جاهزية طبقة الذكاء الاصطناعي | 0% |
| أتمتة الجودة | 12% |
| **الإجمالي** | **28%** |
...
- **`pbac.ts`**: سجل صلاحيات مركزي مع دالة `hasPermission()` ✅
- **`middleware.ts`**: يطبق RBAC على مستوى المسارات ✅
...
### جداول غائبة تماماً من قاعدة البيانات
...
## رابعاً: الأخطاء والمخاطر الحالية

### الخطر 1: قطيعة الهوية — Guardian منفصل عن Auth User
...
### الخطر 2: جدولا `user_roles` و`user_personas` متضاربان
...
### الخطر 3: جدول `events` يستخدم لأغراض متعارضة
...
### الخطر 4: TypeScript أمام قاعدة البيانات
...
### الخطر 5: غياب `terms` كجدول مستقل
...
### الخطر 6: لا `stages` (المراحل) كجدول
...
### الخطر 7: تناقض RLS — بعض Policies تستخدم `profiles.role` المهجور
...
### الخطر 9: تحليلات حية لا كاش — مشكلة أداء مستقبلية
...
### الخطر 10: `health_visits` يفتقر لحقول جوهرية
...
## سادساً: خطة التعديل الآمنة بدون حذف

### المرحلة 0: التوثيق والتحقق (أسبوع واحد)
- [ ] تشغيل `00_preflight_checks.sql` والتحقق من النتائج
- [ ] مراجعة جميع الـ TypeScript types مقابل الجداول الموجودة
- [ ] توثيق الفجوات بالأرقام الدقيقة
- [ ] التأكد من أن جميع الـ RLS policies تستخدم `school_role_type` الحديث
...
| الملف | السبب |
|-------|-------|
| `lib/pbac.ts` | تحديث الصلاحيات لتشمل period_attendance, LRC, health |
| `lib/roles.ts` | التحقق من اكتمال ROLE_DASHBOARD_MAP |
| `lib/context-service.ts` | كيف يُبنى سياق المستخدم — هل يشمل guardian_id؟ |
| `lib/audit-service.ts` | إضافة source_module كحقل مطلوب |
| `middleware.ts` | التحقق من أن ROLE_ACCESS_MAP يغطي /counselor, /qa بشكل صحيح |

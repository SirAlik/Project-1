...
| `20260701_curriculum_progress` (Sprint 7) | **مُطبَّق ثم صُحِّح في Sprint 8.** أنشأ `curriculum_units`/`curriculum_lessons`/`class_curriculum_progress` بنموذج **خاطئ** (منهج عالمي `subject_id+grade_level` يؤلّفه admin). صحّحه `20260702` أدناه. (يُترك كسجلّ تاريخي؛ لا تُعِد تطبيقه فوق التصحيح.) |
| `20260702_curriculum_distribution_correction` (Sprint 8) | **مُطبَّق + متتبَّع حياً.** تصحيح نموذج المنهج إلى **توزيع يؤلّفه المعلّم**: `curriculum_units` أُعيد تحجيمها إلى `(class_id, subject_id)` + `created_by` (UNIQUE class_id,subject_id,title) · `curriculum_lessons` + `planned_date`/`status`/`completed_at`/`notes` (الحالة على الدرس) · **DROP** `class_curriculum_progress` + `is_assigned_class_teacher` · دالتان جديدتان `is_assigned_subject_teacher(uuid,uuid)` + `is_unit_owner_teacher(uuid)` (SECURITY DEFINER). RLS: كتابة units/lessons **للمعلّم المُسنَد فقط** (لا كتابة للإدارة)؛ قراءة لـ system_owner + إدارة المدرسة (متابعة) + المعلّم المُسنَد؛ `school_id=get_my_school_id()`؛ لا USING(true). PRE-LAUNCH 0 صفوف · 0 ERROR advisors. |

Migrations are applied **once, in order**. To fix a mistake: write a new migration. Never edit an already-applied migration.

> **تتبّع الترحيلات (واقع 2026-06-27 · Sprint 8):** **99** ملف ترحيل محلي مقابل **15** إدخالاً متتبَّعاً في `supabase_migrations.schema_migrations` (ارتفع التتبّع لأن ترحيلات Sprint 2–3–7–8 طُبِّقت عبر أداة `apply_migration` التي تُسجّلها) — يبقى الانحراف **bookkeeping تجميلياً** للترحيلات التاريخية المُطبَّقة بـSQL مباشر. المخطط الحيّ مُجسَّد بالكامل: **118 جدول** كلها RLS · **314 سياسة** (انخفض الجدول/السياسات لأن Sprint 8 حذف `class_curriculum_progress` وأعاد بناء سياسات المنهج). **0 كائن مطلوب مفقود.** المصالحة المُوصى بها: **baseline موثّق (الخيار A) ثم repair محدَّد (الخيار B)** — **ممنوع** `supabase db reset`/`db push` على الحيّة. التفاصيل: `docs/db/MIGRATION_TRACKING_AUDIT.md` · ملخّص الانتقال: `docs/audits/CONVERSATION_HANDOFF_SUMMARY.md`.

> **نموذج المنهج الحالي (Sprint 8 · teacher-authored):** `curriculum_units` (نطاق `class_id`+`subject_id`) + `curriculum_lessons` (الحالة على الدرس: `status`/`completed_at`/`planned_date`/`notes`). **حُذف** `class_curriculum_progress`. **RLS:** الكتابة (وحدات/دروس) **للمعلّم المُسنَد فقط** عبر `is_assigned_subject_teacher`/`is_unit_owner_teacher` (SECURITY DEFINER)؛ القراءة لـ system_owner + إدارة المدرسة (متابعة فقط، **لا كتابة**) + المعلّم المُسنَد؛ `school_id = get_my_school_id()`؛ لا `USING(true)`؛ لا وصول عبر-المدارس.

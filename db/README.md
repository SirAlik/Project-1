...
> **تتبّع الترحيلات (واقع 2026-06-27 · Sprint 8):** **99** ملف ترحيل محلي مقابل **15** إدخالاً متتبَّعاً في `supabase_migrations.schema_migrations` (ارتفع التتبّع لأن ترحيلات Sprint 2–3–7–8 طُبِّقت عبر أداة `apply_migration` التي تُسجّلها) — يبقى الانحراف **bookkeeping تجميلياً** للترحيلات التاريخية المُطبَّقة بـSQL مباشر. المخطط الحيّ مُجسَّد بالكامل: **118 جدول** كلها RLS · **314 سياسة** (انخفض الجدول/السياسات لأن Sprint 8 حذف `class_curriculum_progress` وأعاد بناء سياسات المنهج). **0 كائن مطلوب مفقود.** المصالحة المُوصى بها: **baseline موثّق (الخيار A) ثم repair محدَّد (الخيار B)** — **ممنوع** `supabase db reset`/`db push` على الحيّة. التفاصيل: `docs/db/MIGRATION_TRACKING_AUDIT.md`.

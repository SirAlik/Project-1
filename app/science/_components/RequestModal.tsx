...
export function RequestModal({ isOpen, onClose, onSubmit, experiments }: Props) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-200/70 backdrop-blur-sm p-4">
            <div className="w-full max-w-md rounded-2xl border border-cyan-500/20 bg-white p-6 shadow-2xl animate-in fade-in zoom-in duration-200">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-xs text-stone-500 mb-1">التاريخ</label>
                        <input
                            type="date" required
                            value={date} onChange={e => setDate(e.target.value)}
                            className="w-full rounded-xl border border-stone-200 bg-stone-100 p-3 text-sm outline-none focus:border-cyan-500/50"
                            aria-label="التاريخ"
                        />
                    </div>

                    <div>
                        <label className="block text-xs text-stone-500 mb-1">الحصة</label>
                        <select
                    ...
                    <div>
                        <label className="block text-xs text-stone-500 mb-1">التجربة (اختياري)</label>
                        <select
                    ...
                    <div className="pt-4 flex justify-end gap-3">
                        <button type="button" onClick={onClose} className="text-stone-500 text-sm hover:text-foreground transition">إلغاء</button>
                        <button

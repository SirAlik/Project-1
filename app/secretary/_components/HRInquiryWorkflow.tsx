...
export function HRInquiryWorkflow({ inquiries, onUpdateInquiry }: Props) {
    ...
    const getStatusStyles = (status: HRInquiry["status"]) => {
        switch (status) {
            ...
            case "deducted": return "bg-zinc-800 text-zinc-400 border-zinc-700";
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center bg-zinc-900/50 p-6 rounded-[2rem] border border-zinc-800">
                <div>
                    ...
                    <p className="text-zinc-500 text-sm mt-1">إدارة تبريرات التأخر والغياب واتخاذ القرارات الإدارية.</p>
                </div>
                <div className="flex bg-zinc-950 p-1.5 rounded-2xl border border-zinc-800 shrink-0">
                    {[
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${filter === t.id ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                            {t.label}
                        </button>
                    ))}
            ...
            <div className="grid grid-cols-1 gap-4">
                {filtered.map(inquiry => (
                    <div key={inquiry.id} className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-[2.5rem] hover:border-zinc-700 transition-all group">
                        <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
                            ...
                            <div className="flex gap-2 w-full md:w-auto mt-4 md:mt-0">
                                ...
                                {["justified", "deducted"].includes(inquiry.status) && (
                                    <div className="flex items-center gap-2 text-zinc-500 text-[10px] font-bold bg-zinc-950 px-4 py-2 rounded-xl border border-zinc-800">
                        ...
                        {inquiry.justification && (
                            <div className="mt-6 p-4 bg-zinc-950/50 rounded-2xl border border-zinc-800/50">
                                <p className="text-[10px] font-black text-zinc-600 uppercase mb-2">تبرير الموظف:</p>
                                <p className="text-zinc-400 text-xs italic">&quot;{inquiry.justification}&quot;</p>
                            </div>
                        )}
                    </div>
                ))}
                ...
                {filtered.length === 0 && (
                    <div className="bg-zinc-900/50 border border-zinc-800 p-12 rounded-[2.5rem] text-center">
                        ...
                        <h4 className="text-zinc-500 font-bold">لا توجد استفسارات بهذا التصنيف حالياً</h4>
                    </div>
                )}
            </div>
            ...
            {selectedInquiry && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-[3rem] shadow-2xl animate-in zoom-in-95 duration-300">
                        <div className="p-8">
                            <div className="flex justify-between items-start mb-8">
                                <div>
                                    <h4 className="text-white text-xl font-bold">معالجة استفسار</h4>
                                    <p className="text-zinc-500 text-xs mt-1">{selectedInquiry.employee?.name}</p>
                                </div>
                                <button onClick={() => setSelectedInquiry(null)} className="text-zinc-500 hover:text-white transition-colors">إغلاق</button>
                            </div>
                                ...
                                {selectedInquiry.status === "pending_justification" ? (
                                    <>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest px-1">تبرير الموظف</label>
                                            <textarea name="justification" className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-4 text-xs text-white font-bold h-24" required aria-label="تبرير الموظف" />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest px-1">قرار المدير</label>
                                            <select name="decision" className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-4 text-xs text-white font-bold appearance-none" aria-label="قرار المدير">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest px-1">عدد أيام الحسم</label>
                                            <input name="days" type="number" step="0.5" className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-4 text-xs text-white font-bold" required defaultValue="1" aria-label="عدد أيام الحسم" />
                                        </div>
                                    </div>
                                )}

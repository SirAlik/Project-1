...
function DraggableAssignment({ id, type, label, icon }: { id: string, type: 'subject' | 'teacher', label: string, icon: React.ReactNode }) {
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col">
                ...
                {type === 'teacher' && <span className="text-[8px] text-muted-foreground tracking-wide uppercase">Teacher Verified</span>}
            </div>
        </div>
    );
}

function TimetableSlot({ id, day, period, content, conflict }: { id: string, day: number, period: number, content?: SlotContent, conflict?: string }) {
...
interface SubjectItem { id: string; name_ar: string; }
interface TimetableSlotRow { class_id: string; teacher_id: string; subject_id: string; day: number; period: number; }
...
export default function TimetablePage() {
    ...
    const [allSlots, setAllSlots] = useState<TimetableSlotRow[]>([]);
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const { data: sl } = await supabase.from('timetable_slots').select('*');
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (dragData.type === 'teacher') {
            const isBusy = allSlots.find(s => s.teacher_id === dragData.id && s.day === day && s.period === period && s.class_id !== selectedClass?.id);
            if (isBusy) {
                ...
                <div className="max-w-[1700px] mx-auto space-y-12">
                    ...
                    <header className="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div>
                            ...
                            <p className="text-muted-foreground text-xs font-bold mt-2 uppercase tracking-[0.3em]">Neural Schedule Optimizer v4.2</p>
                        </div>

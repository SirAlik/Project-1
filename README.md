...
### ⚠️ تحذيرات غير حرجة في آخر بناء (non-fatal warnings)

`npm run build` ينجح بـ 61/61 صفحة بدون أخطاء، لكنه يُصدر تحذيرَين غير حرجَين:

* **`middleware.ts` convention deprecated** — Next.js 16 ينبّه بأن الاصطلاح الحالي لـ `middleware.ts` مُهمَل وسيُستبدل بـ `proxy.ts` في إصدارات مستقبلية. لم يُنفَّذ الترحيل بعد.
* **Recharts `width`/`height` warning** — مكوّنات الرسوم البيانية في صفحات analytics تُظهر تحذير "width/height not set" عند prerender. غير مؤثر على الوظيفة.
